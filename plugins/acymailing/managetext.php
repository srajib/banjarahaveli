...
class plgAcymailingManagetext extends JPlugin
{
	...
	function _getAllEmails(){
		if(!empty($this->allEmails)) return $this->allEmails;
		$db =& JFactory::getDBO();
		$db->setQuery("SELECT `mailid`,CONCAT(`subject`,' ( ',`mailid`,' )') as 'value' FROM `#__acymailing_mail` ORDER BY `subject` ASC ");
		$this->allEmails = $db->loadObjectList();
		return $this->allEmails;
	}
	 function onAcyDisplayActions(&$type){
	 	$type['addqueue'] = JText::_('ADD_QUEUE');
	 	$type['removequeue'] = JText::_('REMOVE_QUEUE');
	 	$addqueue = '<div id="action__num__addqueue">'.JHTML::_('select.genericlist',  $this->_getAllEmails(), "action[__num__][addqueue][mailid]", 'class="inputbox" size="1"', 'mailid', 'value').'<br/><label for="addqueuesenddate__num__">'.JText::_('SEND_DATE').' </label> <input value="{time}" id="addqueuesenddate__num__" name="action[__num__][addqueue][senddate]" /></div>';
	 	$removequeue = '<div id="action__num__removequeue">'.JHTML::_('select.genericlist',  $this->_getAllEmails(), "action[__num__][removequeue][mailid]", 'class="inputbox" size="1"', 'mailid', 'value').'</div>';
