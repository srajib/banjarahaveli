...
class plgSystemRegacymailing extends JPlugin
{
	...
	function onAfterRender(){
		...
		$view = JRequest::getString('view',JRequest::getString('ctrl',JRequest::getString('task')));
		...
		$components['com_hikashop'] = array('view' => 'checkout','lengthafter' => 500 , 'tdclass' => 'key', 'email' => 'data\[register\]\[email\]','password' => 'data\[register\]\[password2\]');
		...
		if(preg_match('#(name="'.$after.'".{0,'.$components[$option]['lengthafter'].'}</tr>)#Uis',$body)){
			$return = '<tr class="acysubscribe"><td '.$tdclass.' style="padding-top:5px" valign="top">'.$subText.'</td><td><table>'.$return.'</table></td></tr>';
