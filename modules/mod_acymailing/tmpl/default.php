				...
				<tr>
					<?php foreach($fieldsToDisplay as $oneField){
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							...
							if($displayOutside) echo '<td><span class="user_email"><label for="user_email_'.$formName.'">'.$emailCaption.'</label></span></td>'; ?>
							</tr>
							<tr><td>
				...
				if(empty($identifiedUser->userid) AND $config->get('captcha_enabled') AND acymailing::level(1)){ ?>
				 ...
				 if($params->get('showterms',false)){
					...
					<td <?php if($displayOutside AND !$displayInline) echo 'colspan="2"'; ?> class="acysubbuttons">
						...
						<input id="subscription_button" class="button" type="submit" value="<?php $subtext = $params->get('subscribetextreg'); if(empty($identifiedUser->userid) OR empty($subtext)){ $subtext = $params->get('subscribetext',JText::_('SUBSCRIBECAPTION')); } //echo $subtext;  ?>" name="Submit" onclick="try{ return submitacymailingform('optin','<?php echo $formName;?>'); }catch(err){alert('The form could not be submitted, please make sure the jdoc:include type=head tag is in your template');return false;}"/>
