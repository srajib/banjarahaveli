<?php
  ...
  if ($Corder != $cs ) # new category
  {
    ...
    $content .= "\n<li class=\"cats".$moduleclass_sfx."\">";
    if ($dropdown){ $content .= "<span class=\"caticon\">&nbsp;</span>";}
    if ($showheader) { $content .= "<span class=\"cat_title".$moduleclass_sfx."\">".htmlspecialchars($row->ctitle)."</span>"; }
    ...
    if ($showcdes) {
      //ugly workaround for line breakes that are inserted by editors in the Category descriptions (done for: Tiny, FCK and none)
    ...
    $content .="\n<ul class=\"links".$moduleclass_sfx."\">\n";
    $cs = $row->catid;
  }
    $trow+=1;
    ...
    if ($lnkicon == "-1") {
      $content .= "<li><a class=\"link".$moduleclass_sfx."\" ".$target." ".$title." href=\"".$URL."\">".$row->title."</a>";
    } else {
      $content .= "<li><a class=\"link".$moduleclass_sfx."\" style=\"background: url('".$relpath."/images/M_images/".$lnkicon."') no-repeat; padding: 0 0 5px 25px;\" ".$target." ".$title." href=\"".$URL."\">".$row->title."</a>";
    }
    /* show hits */
